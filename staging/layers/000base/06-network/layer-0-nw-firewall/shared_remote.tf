...
data "terraform_remote_state" "cloudwatch-logs" {
  backend = "s3"
  config = {
    bucket = "abaxx-exch-tf-state-nonprod"
...
data "terraform_remote_state" "cloudwatch-logs-role" {
  backend = "s3"
  config = {
    bucket = "abaxx-exch-tf-state-nonprod"
...
data "terraform_remote_state" "network-firewall-rules-aws-staging" {
  backend = "s3"
  config = {
    bucket = "abaxx-exch-tf-state-nonprod"
    key    = "abaxxsingapore/abex-aws-cross-network-firewall-rules/environments/aws-staging/terraform.tfstate"
    region = "ap-southeast-1"
  }
}
