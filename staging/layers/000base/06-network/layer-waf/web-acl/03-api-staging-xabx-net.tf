##########################################
# api.staging.xabx.net 
##########################################

resource "aws_wafv2_web_acl" "api_staging_cloudfront_cdn_web_acl" {
  name        = "api_staging_cloudfront_cdn_web_acl"
  scope       = "CLOUDFRONT"
  description = "Web ACL to be attached to api Staging CloudFront CDN"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "api_staging_cloudfront_cdn_web_acl"
    sampled_requests_enabled   = true
  }
