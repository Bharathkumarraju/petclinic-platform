##########################################
# clearing-backend.staging.abaxx.exchange
##########################################

resource "aws_wafv2_web_acl" "clearing_backend_staging_cloudfront_cdn_web_acl" {
  name        = "clearing_backend_staging_cloudfront_cdn_web_acl"
  scope       = "CLOUDFRONT"
  description = "Web ACL to be attached to clearing-backend Staging CloudFront CDN"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "clearing_backend_staging_cloudfront_cdn_web_acl"
    sampled_requests_enabled   = true
  }
