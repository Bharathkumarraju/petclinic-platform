##########################################
# atrp-backend.staging.xabx.net 
##########################################

resource "aws_wafv2_web_acl" "atrp_backend_staging_cloudfront_cdn_web_acl" {
  name        = "atrp_backend_staging_cloudfront_cdn_web_acl"
  scope       = "CLOUDFRONT"
  description = "Web ACL to be attached to ATRP Backend Staging CloudFront CDN"
  ...
  rule {
    name     = "AllowAbaxxExchangeNATGWExternal"
    priority = 9
    action {
      allow {} # Allows if the IP matches the set
    }
    statement {
      ip_set_reference_statement {
        arn = data.terraform_remote_state.ip-set.outputs.abaxx_exchange_nat_gw_external_arn
      }
    }
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "AllowAbaxxExchangeNATGWExternal"
      sampled_requests_enabled   = true
    }
  }
  rule {
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "atrp_backend_staging_cloudfront_cdn_web_acl"
    sampled_requests_enabled   = true
  }
