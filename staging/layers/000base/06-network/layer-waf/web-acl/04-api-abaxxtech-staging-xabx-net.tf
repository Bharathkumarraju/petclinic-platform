##########################################
# api-abaxxtech.staging.xabx.net 
##########################################

resource "aws_wafv2_web_acl" "api_abaxxtech_staging_cloudfront_cdn_web_acl" {
  name        = "api_abaxxtech_staging_cloudfront_cdn_web_acl"
  scope       = "CLOUDFRONT"
  description = "Web ACL to be attached to api-abaxxtech Staging CloudFront CDN"
  ...
  rule {
    ...
    statement {
      ip_set_reference_statement {
        arn = data.terraform_remote_state.ip-set.outputs.verifier_staging_arn
      }
    }
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "AllowAbaxxVerifier"
      sampled_requests_enabled   = true
    }
  }
  rule {
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "api_abaxxtech_staging_cloudfront_cdn_web_acl"
    sampled_requests_enabled   = true
  }
