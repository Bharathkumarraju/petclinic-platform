...
data "terraform_remote_state" "exchangedb" {
  backend = "s3"
  config = {
    bucket = "abaxx-exch-tf-state-nonprod"
    key    = "abaxxsingapore/abex-aws-env-${local.env}/layers/100data/04-aurora/aurora-mysql/terraform.tfstate"
    region = "ap-southeast-1"
  }
}

data "terraform_remote_state" "riskdb" {
  backend = "s3"
  config = {
    bucket = "abaxx-exch-tf-state-nonprod"
    key    = "abaxxsingapore/abex-aws-env-${local.env}/layers/100data/04-aurora/aurora-mysql-risk/terraform.tfstate"
    region = "ap-southeast-1"
  }
}
