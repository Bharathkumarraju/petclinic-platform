locals {
  ...
  riskdba_username                       = nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.risk-db-creds.secret_string)["username"])
  riskdba_password                       = jsondecode(data.aws_secretsmanager_secret_version.risk-db-creds.secret_string)["password"]
  risk_username                          = nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.risk-user-creds.secret_string)["username"])
  riskviewer_username                    = nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.risk-viewer-creds.secret_string)["username"])
