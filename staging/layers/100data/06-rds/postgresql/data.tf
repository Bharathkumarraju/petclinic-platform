...
###### Reading TF state for infra (VPC, subnet) #######

# data "terraform_remote_state" "infra" {
#   backend = "s3"
#   config = {
#     bucket = "abaxx-exch-tf-state-nonprod"
#     key    = "abaxxsingapore/infra/base/network/layer-0/terraform.tfstate"
#     region = "ap-southeast-1"
#   }
# }
...
data "terraform_remote_state" "secrets-kms-keys" {
  ...
  config = {
    bucket = "abaxx-exch-tf-state-nonprod"
    key    = "abaxxsingapore/abex-aws-env-${local.env}/layers/000base/01-kms/secrets_manager/terraform.tfstate" #"abaxxsingapore/abex-aws-cross-secrets/aws-${local.env}/kms"#"abaxxsingapore/abex-aws-env-${local.env}/layers/000base/01-kms/secrets_manager/terraform.tfstate"
    region = "ap-southeast-1"
  }
}
