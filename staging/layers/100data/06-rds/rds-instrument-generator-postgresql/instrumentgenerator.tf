module "rds_instrumentgenerator" {
...
# PostgreSQL Database Configuration for instrumentgenerator
# This module creates database roles, schemas, and access control
module "rds_instrumentgenerator_config" {
  ...
  database_name = "adw_instruments"
  schema_name   = "adw_instruments"

  # RDS instance connection details
  master_username      = module.rds_instrumentgenerator.db_instance_username
  db_instance_endpoint = module.rds_instrumentgenerator.db_instance_address
  ...
  depends_on = [module.rds_instrumentgenerator]
}
