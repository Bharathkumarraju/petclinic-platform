...
data "aws_db_instance" "instrumentgenerator" {
  db_instance_identifier = local.db_identifier
}

data "aws_secretsmanager_secret_version" "instrumentgenerator_master" {
  secret_id = data.aws_db_instance.instrumentgenerator.master_user_secret[0].secret_arn
}