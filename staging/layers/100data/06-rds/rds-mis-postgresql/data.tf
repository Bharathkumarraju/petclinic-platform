...
data "aws_db_instance" "datawarehouse" {
  db_instance_identifier = local.db_identifier
}

data "aws_secretsmanager_secret_version" "datawarehouse_master" {
  secret_id = data.aws_db_instance.datawarehouse.master_user_secret[0].secret_arn
}
